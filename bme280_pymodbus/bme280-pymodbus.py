...
def updating_writer(a):
    ...
    cTempInt = int(cTemp)
    cTempDec = round((cTemp - cTempInt) * 10000)
    fTempInt = int(fTemp)
    fTempDec = round((fTemp - fTempInt) * 10000)
    pressureInt = int(pressure)
    pressureDec = round((pressure - pressureInt) * 10000)
    humidityInt = int(humidity)
    humidityDec = round((humidity - humidityInt) * 10000)
    ...
    values[2] = fTempInt
    values[3] = fTempDec
    values[4] = pressureInt
    values[5] = pressureDec
    values[6] = humidityInt
    values[7] = humidityDec
